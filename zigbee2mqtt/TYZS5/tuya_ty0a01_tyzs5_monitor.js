// TZLL Zigbee2MQTT external converter — TYZS5 Monitor Edition.
// v0.1.0 — read-only integration for firmware identity Tuya / TY0A01-TYZS5-MON.
//
// Security boundary:
//   * No lock-control toZigbee converter is registered.
//   * All TZLL exposes are incoming state/event/diagnostic data only.
//   * The matching Monitor firmware independently rejects externally-originated
//     lock-control DPs, giving defense in depth at integration + firmware layers.
//
// This converter deliberately omits Control-only functionality: remote unlock,
// temporary-password management, credential enrollment/deletion, password-free
// key provisioning and any raw/generic DP writer.
...
const parseShardBitmapIds = (value) => {
    ...
    // Exact empty-list sentinel observed on tested TYZS3/TYZS5 locks.
...
const definitions = [{
    fingerprint: [{manufacturerName: 'Tuya', modelID: 'TY0A01-TYZS5-MON'}],
    model: 'TY0A01-TYZS5-MON',
    vendor: 'Tuya',
    description: 'TZLL TYZS5 Monitor Edition v0.1.0 — read-only Zigbee2MQTT integration',
    ...
    exposes: [
        ...
        e.text('alarm', ea.STATE)
            .withDescription('MCU-reported DP9 alarm state.'),
        e.text('open_close', ea.STATE)
            .withLabel('DP8 诊断状态')
            .withDescription('Diagnostic lock/door state reported by the lock MCU. Not a reliable independent physical door-contact sensor.')
    ...
    meta: {
        tuyaDatapoints: [
            ...
            [8, 'open_close', scalarText],
            [9, 'alarm', {from: (value) => ALARM_LOOKUP[value] !== undefined ? ALARM_LOOKUP[value] : `alarm_${value}`}],
