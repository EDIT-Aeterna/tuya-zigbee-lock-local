...
bool emberAfPreCommandReceivedCallback(EmberAfClusterCommand *cmd)
{
  ...
  if (cmd != NULL && cmd->apsFrame != NULL
    ...
    /* Consume the legacy control channel without entering firmware control code. */
    return true;
#else
  ...
  if (cmd != NULL && cmd->apsFrame != NULL
    ...
    /* MCU telemetry is outbound EF00; consume all inbound EF00 control commands. */
    return true;
#else
    uint16_t plen = (cmd->bufLen > cmd->payloadStartIndex)
                    ? (uint16_t)(cmd->bufLen - cmd->payloadStartIndex) : 0;
  ...
  if (cmd != NULL && cmd->apsFrame != NULL
      ...
      && cmd->clusterSpecific) {
#if KAGEL_TELEMETRY_ONLY
    /* No OTA client is present in the telemetry-only profile. */
    return true;
#else
    uint16_t plen = (cmd->bufLen > cmd->payloadStartIndex)
                    ? (uint16_t)(cmd->bufLen - cmd->payloadStartIndex) : 0;
