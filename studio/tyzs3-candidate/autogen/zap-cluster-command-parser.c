...
1u)
  ...
  // - Argument is not present in all versions.
  if (payloadOffset >= cmd->bufLen || cmd->bufLen < payloadOffset + emberAfStringLength(cmd->buffer + payloadOffset) + 1u)
  {
...
1u ||
 !((cmd_struct->options & 0x0018) == 0x0010))
  ...
  if ( (cmd_struct->options & 0x0018) == 0x0010 ) {
    ...
    if (cmd->bufLen < payloadOffset + cmd_struct->groupListCount * sizeof(GpPairingConfigurationGroupList)) {
      return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
    }
...
1u)
  ...
  if ( 0 < cmd_struct->numberOfPairedEndpoints && cmd_struct->numberOfPairedEndpoints < 0xFD ) {
    ...
    if (cmd->bufLen < payloadOffset + cmd_struct->numberOfPairedEndpoints * sizeof(uint8_t)) {
      return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
    }
...
1u)
  ...
  if ( (cmd_struct->options & 0x0400) && (cmd_struct->applicationInformation & 0x04) ) {
    ...
    if (cmd->bufLen < payloadOffset + cmd_struct->numberOfGpdCommands * sizeof(uint8_t)) {
      return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
    }
...
1u)
  ...
  if ( (cmd_struct->options & 0x0400) && (cmd_struct->applicationInformation & 0x08) && ((cmd_struct->clusterIdListCount & 0x0F) > 0) ) {
    ...
    if (cmd->bufLen < payloadOffset + (cmd_struct->clusterIdListCount & 0x0F) * sizeof(uint16_t)) {
      return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
    }
  ...
  if ( (cmd_struct->options & 0x0400) && (cmd_struct->applicationInformation & 0x08) && ((cmd_struct->clusterIdListCount >> 4) > 0) ) {
    ...
    if (cmd->bufLen < payloadOffset + (cmd_struct->clusterIdListCount >> 4) * sizeof(uint16_t)) {
      return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
    }
...
1u)
      ...
      // - Argument is conditionally present based on expression: (cmd_struct->actions & 0x07) == 0x05
      if (!((cmd_struct->actions & 0x07) == 0x05)) {
...
1u)
  ...
  if ( (cmd_struct->actions & 0x07) == 0x05 ) {
    // Array is conditionally present based on expression: (cmd_struct->actions & 0x07) == 0x05
...
1u)
  {
    return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
        ...
        payloadOffset+= 1u;
    if (cmd->bufLen < payloadOffset + cmd_struct->numberOfZones * sizeof(uint8_t)) {
      return EMBER_ZCL_STATUS_MALFORMED_COMMAND;
    }
