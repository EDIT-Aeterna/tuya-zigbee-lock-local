...
/** @brief Legacy inbound remote datapoint request.
* Cluster: Kagel EF00 Telemetry, Kagel lock MCU telemetry and legacy inbound command descriptor.
* Command: DataRequest
*/
#define emberAfFillCommandKagelEf00ClusterDataRequest( \
) \
emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND \
                           | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER), \
                           ZCL_KAGEL_EF00_CLUSTER_ID, \
                           ZCL_DATA_REQUEST_COMMAND_ID, \
                           "" \
);



/** @brief Outbound lock MCU telemetry report.
* Cluster: Kagel EF00 Telemetry, Kagel lock MCU telemetry and legacy inbound command descriptor.
* Command: DataReport
*/
#define emberAfFillCommandKagelEf00ClusterDataReport( \
) \
emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND \
                           | ZCL_FRAME_CONTROL_SERVER_TO_CLIENT), \
                           ZCL_KAGEL_EF00_CLUSTER_ID, \
                           ZCL_DATA_REPORT_COMMAND_ID, \
                           "" \
);



/** @brief Legacy inbound remote datapoint command.
* Cluster: Kagel EF00 Telemetry, Kagel lock MCU telemetry and legacy inbound command descriptor.
* Command: SendData
*/
#define emberAfFillCommandKagelEf00ClusterSendData( \
) \
emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND \
                           | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER), \
                           ZCL_KAGEL_EF00_CLUSTER_ID, \
                           ZCL_SEND_DATA_COMMAND_ID, \
                           "" \
);



/** @brief Legacy vendor remote datapoint command.
* Cluster: Kagel FC00 Legacy, Legacy Kagel vendor command receiver; telemetry-only firmware consumes it.
* Command: RemoteDp
*/
#define emberAfFillCommandKagelFc00ClusterRemoteDp( \
) \
emberAfFillExternalManufacturerSpecificBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_MANUFACTURER_SPECIFIC_MASK \
                           | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER), \
                           ZCL_KAGEL_FC00_CLUSTER_ID, \
4169, \
                           ZCL_REMOTE_DP_COMMAND_ID, \
                           "" \
);



/** @brief Legacy vendor temporary-password command.
* Cluster: Kagel FC00 Legacy, Legacy Kagel vendor command receiver; telemetry-only firmware consumes it.
* Command: CreateTempPassword
*/
#define emberAfFillCommandKagelFc00ClusterCreateTempPassword( \
) \
emberAfFillExternalManufacturerSpecificBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_MANUFACTURER_SPECIFIC_MASK \
                           | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER), \
                           ZCL_KAGEL_FC00_CLUSTER_ID, \
4169, \
                           ZCL_CREATE_TEMP_PASSWORD_COMMAND_ID, \
                           "" \
);
